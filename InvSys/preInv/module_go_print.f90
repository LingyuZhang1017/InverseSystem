...
contains
  ...
  subroutine GO_Print_Init( status, apply, prompt_pe, pe, trace, file, file_name )
    ...
    use module_go_fu, only : goStdOut
  ...
  subroutine goPr
    ...
    integer             ::  nind  !nind:number of spaces to indent
    ...
    if ( pr_apply ) then
      ...
      ! empty output buffer:
      ! (non-standard Fortran routine;
      !  just skip if not available in your impplementation)
#ifdef MIPSpro
#else
      call Flush( pr_fu )
#endif
  ...
  subroutine goErr
    ...
    if ( len_trim(gol) == 0 ) then
      ...
      ! error message:
      write (gol,'("ERROR - ",a)') trim(gol); call goPr
